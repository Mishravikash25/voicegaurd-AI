"""
VoiceGuard AI - Scenario 3 & 4 Screenshot Generator (Hidden Input Handling).
"""
import os
import sys
import time
...
def run():
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_selector('input[type="file"][multiple]', state="attached")
        dataset_input = page.locator('input[type="file"][multiple]').first
        dataset_input.set_input_files([alice_ref, bob_ref, charlie_ref])
        dataset_input.evaluate("el => el.dispatchEvent(new Event('change', { bubbles: true }))")
        time.sleep(1)

        suspect_input = page.locator('input[type="file"]:not([multiple])').first
        suspect_input.set_input_files(alice_auth)
        suspect_input.evaluate("el => el.dispatchEvent(new Event('change', { bubbles: true }))")
        time.sleep(1)
        ...
        single_input = page.locator('input[type="file"]').first
